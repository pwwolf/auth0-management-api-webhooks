...
module.exports = storage => (req, res, next) => {
  ...
  const isCron =
    ...
    (wtHead.referer === "https://manage.auth0.com/" && wtHead["if-none-match"]);
  ...
  const onLogsReceived = (logs, callback) => {
    ...
    endpointsFilter =
      endpointsFilter.length > 0 && endpointsFilter[0] === ""
        ? []
        : endpointsFilter;
    ...
    const requestMatchesFilter = log => {
      ...
      return (
        path &&
        endpointsFilter.some(filter => path.indexOf(`/api/v2/${filter}`) >= 0)
      );
    };

    const filteredLogs = logs.filter(requestMatchesFilter).map(log => ({
      date: log.date,
